///
/// QuantityDataPoint.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2026 Marc Rousavy @ Margelo
///

#pragma once

#if __has_include(<NitroModules/JSIConverter.hpp>)
#include <NitroModules/JSIConverter.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/NitroDefines.hpp>)
#include <NitroModules/NitroDefines.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/JSIHelpers.hpp>)
#include <NitroModules/JSIHelpers.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif



#include <string>
#include <chrono>
#include <unordered_map>
#include <optional>

namespace margelo::nitro::healthkit {

  /**
   * A struct which can be represented as a JavaScript object (QuantityDataPoint).
   */
  struct QuantityDataPoint {
  public:
    double value     SWIFT_PRIVATE;
    std::string unit     SWIFT_PRIVATE;
    std::chrono::system_clock::time_point startDate     SWIFT_PRIVATE;
    std::chrono::system_clock::time_point endDate     SWIFT_PRIVATE;
    std::optional<std::unordered_map<std::string, std::string>> metadata     SWIFT_PRIVATE;

  public:
    QuantityDataPoint() = default;
    explicit QuantityDataPoint(double value, std::string unit, std::chrono::system_clock::time_point startDate, std::chrono::system_clock::time_point endDate, std::optional<std::unordered_map<std::string, std::string>> metadata): value(value), unit(unit), startDate(startDate), endDate(endDate), metadata(metadata) {}
  };

} // namespace margelo::nitro::healthkit

namespace margelo::nitro {

  // C++ QuantityDataPoint <> JS QuantityDataPoint (object)
  template <>
  struct JSIConverter<margelo::nitro::healthkit::QuantityDataPoint> final {
    static inline margelo::nitro::healthkit::QuantityDataPoint fromJSI(jsi::Runtime& runtime, const jsi::Value& arg) {
      jsi::Object obj = arg.asObject(runtime);
      return margelo::nitro::healthkit::QuantityDataPoint(
        JSIConverter<double>::fromJSI(runtime, obj.getProperty(runtime, "value")),
        JSIConverter<std::string>::fromJSI(runtime, obj.getProperty(runtime, "unit")),
        JSIConverter<std::chrono::system_clock::time_point>::fromJSI(runtime, obj.getProperty(runtime, "startDate")),
        JSIConverter<std::chrono::system_clock::time_point>::fromJSI(runtime, obj.getProperty(runtime, "endDate")),
        JSIConverter<std::optional<std::unordered_map<std::string, std::string>>>::fromJSI(runtime, obj.getProperty(runtime, "metadata"))
      );
    }
    static inline jsi::Value toJSI(jsi::Runtime& runtime, const margelo::nitro::healthkit::QuantityDataPoint& arg) {
      jsi::Object obj(runtime);
      obj.setProperty(runtime, "value", JSIConverter<double>::toJSI(runtime, arg.value));
      obj.setProperty(runtime, "unit", JSIConverter<std::string>::toJSI(runtime, arg.unit));
      obj.setProperty(runtime, "startDate", JSIConverter<std::chrono::system_clock::time_point>::toJSI(runtime, arg.startDate));
      obj.setProperty(runtime, "endDate", JSIConverter<std::chrono::system_clock::time_point>::toJSI(runtime, arg.endDate));
      obj.setProperty(runtime, "metadata", JSIConverter<std::optional<std::unordered_map<std::string, std::string>>>::toJSI(runtime, arg.metadata));
      return obj;
    }
    static inline bool canConvert(jsi::Runtime& runtime, const jsi::Value& value) {
      if (!value.isObject()) {
        return false;
      }
      jsi::Object obj = value.getObject(runtime);
      if (!nitro::isPlainObject(runtime, obj)) {
        return false;
      }
      if (!JSIConverter<double>::canConvert(runtime, obj.getProperty(runtime, "value"))) return false;
      if (!JSIConverter<std::string>::canConvert(runtime, obj.getProperty(runtime, "unit"))) return false;
      if (!JSIConverter<std::chrono::system_clock::time_point>::canConvert(runtime, obj.getProperty(runtime, "startDate"))) return false;
      if (!JSIConverter<std::chrono::system_clock::time_point>::canConvert(runtime, obj.getProperty(runtime, "endDate"))) return false;
      if (!JSIConverter<std::optional<std::unordered_map<std::string, std::string>>>::canConvert(runtime, obj.getProperty(runtime, "metadata"))) return false;
      return true;
    }
  };

} // namespace margelo::nitro
