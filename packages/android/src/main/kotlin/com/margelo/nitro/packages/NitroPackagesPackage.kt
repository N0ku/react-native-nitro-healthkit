 ...
 *  1. Trigger [NitroPackagesOnLoad.initializeNative] so `libNitroPackages.so` is loaded
 ...
 */
class NitroPackagesPackage : ReactPackage {
    ...
    private fun ensureInitialised(reactContext: ReactApplicationContext) {
        ...
        synchronized(lock) {
            if (initialised) return
            try {
                NitroPackagesOnLoad.initializeNative()
                HealthConnectContextHolder.attach(reactContext.applicationContext)
                initialised = true
                Log.i(TAG, "NitroPackages native library initialised")
            } catch (t: Throwable) {
                ...
                Log.e(TAG, "Failed to initialise NitroPackages", t)
            }
        }
    }

    companion object {
        private const val TAG = "NitroPackagesPackage"
